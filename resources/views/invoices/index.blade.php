...
<div class="panel">
    ...
    <div class="table-responsive"><table class="table align-middle"><thead><tr><th>Invoice</th><th>Buyer</th><th>Status</th><th>Editable Until</th><th>Total</th><th></th></tr></thead><tbody>@forelse($invoices as $invoice)<tr><td><a href="{{ route('invoices.show', $invoice) }}">{{ $invoice->invoice_number }}</a></td><td>{{ $invoice->buyer_name }}</td><td><span class="status-pill status-{{ $invoice->status->value }}">{{ ucfirst($invoice->status->value) }}</span></td><td>{{ $invoice->editable_until?->format('d M Y H:i') ?: 'N/A' }}</td><td>PKR {{ number_format($invoice->grand_total, 2) }}</td><td class="text-end"><div class="d-inline-flex gap-2"><a href="{{ route('invoices.show', $invoice) }}" class="btn btn-sm btn-outline-light">View</a>@if(auth()->user()?->isManagingClient())<form method="POST" action="{{ route('invoices.destroy', $invoice) }}" onsubmit="return confirm('Delete this invoice? The record will be hidden but kept in the database.');">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger">Delete</button></form>@endif</div></td></tr>@empty<tr><td colspan="6" class="text-secondary">No invoices found.</td></tr>@endforelse</tbody></table></div>
